...
export const MS2DAYS    = (ms:number) => ms/MS_PER_DAY;
export const DAYS2MS    = (days:number) => days*MS_PER_DAY;
export const MS2YEARS   = (ms:number) => ms/MS_PER_YEAR;
export const YEARS2MS   = (y:number)  => y*MS_PER_YEAR;
...
export function isLastBusinessDayOfMonth(date:Date, holidays:string[]) {
   const isHoliday = (d:Date) => holidays.includes(shortDateString(d))
   const isWeekend = (d:Date) => d.getDay()===6 || d.getDay()===0
   const nextBusinessDay = new Date(date)
   do nextBusinessDay.setDate(nextBusinessDay.getDate()+1)
