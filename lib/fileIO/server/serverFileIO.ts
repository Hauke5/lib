...
export type ServerWriteOptions = {
   /** apply versioning; @see {@link Versioning} */
 ...
 * - `file` attempt to breach the sandboxed `appKey` directory
 */
export async function serverPathInfo(appKey:string, filePath:string):Promise<FilePathInfo> {
   try {
 ...
 * - `file` attempt to breach the sandboxed `appKey` directory
 */
export async function serverIsFile(appKey:string, file:string):Promise<boolean> {
   try {
      const fileName = checkForValidAppPath(appKey, file)
      const {perm} = await hasPermission(fileName)
 ...
 * - reading `file` is denied for the currently authenticated user
 */
export async function serverIsDir(appKey:string, path:string):Promise<boolean> {
   try {
 ...
 */
export async function serverMove(appKey:string, fromPath:string, toPath:string, versioning:Versioning=false):Promise<true> {
   try {
 ...
 */
export async function serverRemove(appKey:string, file:string, versioning:Versioning=false):Promise<true> {
   try {
 ...
 * - reading `file` is denied for the currently authenticated user
 */
export async function serverJsonRead<RETURN=any>(appKey:string, file:string):Promise<RETURN> {
   try {
 ...
 * - reading `file` is denied for the currently authenticated user
 */
export async function serverTextRead(appKey:string, file:string):Promise<string> {
   try {
      const fileName = await passReadPermission(appKey, file)
 ...
 * - reading `file` is denied for the currently authenticated user
 */
export async function serverBinRead(appKey:string, file:string):Promise<string> {
   try {
      const fileName = await passReadPermission(appKey, file)
 ...
 */
export async function serverJsonWrite(appKey:string, file:string, content:any, {versioning=false, notModifiedAfter}:ServerWriteOptions={}):Promise<number|null> {
   try {
      const fileName = await passWritePermission(appKey, file)
 ...
 * @see {@link ServerWriteOptions}, {@link Versioning}, 
 */
export async function serverTextWrite(appKey:string, file:string, content='', {versioning=false, append=false, notModifiedAfter}:ServerWriteOptions={}):Promise<number|null> {
   try {
      const fileName = await passWritePermission(appKey, file)
      if (await wasModifiedAfter(appKey, file, notModifiedAfter)) {
...
async function wasModifiedAfter(appKey:string, file:string, notModifiedAfter?:Date) {
   if (!notModifiedAfter) return false
 ...
 */
function checkForValidAppPath(appKey:string, file:string) {
 ...
 */
async function passReadPermission(appKey:string, file:string):Promise<string> {
   const fileName = checkForValidAppPath(appKey, file)
   const {perm} = await hasPermission(fileName)
   if (!perm.read) {
 ...
 */
export async function passWritePermission(appKey:string, file:string):Promise<string> {
   const fileName = checkForValidAppPath(appKey, file)
...
function updatePermissions(permissions:FilePermissions, accessFile:string) {
   ...
   function initPermissionType(type:string, accessSpec:AccessFileSpec) {
      if (accessSpec[type] && permissions[ALL_USERS][type]===undefined) {
         ...
         accessSpec[type].forEach((user:string) => permissions[user] ??= {user})
      }
   }
}
 ...
 */
function checkAndCreateVersioning(file:string, versioning:Versioning) {
